...
class Level06 extends Phaser.Scene{
    ...
    create(){
        ...
        this.player.addCollider(layers.layer_brambles, this.player.respawn); 
    ...
    //Creation de la map
    createMap(){
        const map = this.make.tilemap({key: "level_06"});
    ...
    checkFirePlaceWatered(particle){
        this.firePlaces.children.each(function(fire) {
            if((particle.x > fire.x - fire.width/2 && particle.x < fire.x + fire.width/2) && (particle.y > fire.y && particle.y < fire.y + fire.height) ){
                particle.lifeCurrent = 0; 
                fire.extinguish();  
            }
        }, this);
